...
import { LoadingState } from '../../components/portal/ui/page-chrome'
import { useProfileForm } from '../../hooks/use-profile-form'
import { useAuthStore } from '../../store/auth-store'
import { useProfileStore } from '../../store/profile-store'
import { isPortalProfileComplete } from '../../utils/profile-helpers'

interface ProfilePageProps {
  portal: 'director' | 'staff'
  homePath: string
}
...
export function ProfilePage({ portal, homePath }: ProfilePageProps) {
  const user = useAuthStore((s) => s.user)
  const { profile, loading, fetchDirectorProfile, fetchStaffProfile } = useProfileStore()
  const [modalOpen, setModalOpen] = useState(false)

  const isDirector = portal === 'director' || user?.role === 'director'
  ...
  useEffect(() => {
    if (portal === 'director') {
      void fetchDirectorProfile()
    } else {
      void fetchStaffProfile()
    }
  }, [fetchDirectorProfile, fetchStaffProfile, portal])
