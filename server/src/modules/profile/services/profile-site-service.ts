...
import { PortalProfileDto, ProfileAssetKind, UpdatePortalProfileDto } from '../dto/profile-site-dto';
...
export class ProfileSiteService {
    ...
    async updateMyProfile(
        ...
        const payload: UpdatePortalProfileDto & {
            avatar?: string | null;
    ...
    async getProfileAsset(userId: string, kind: ProfileAssetKind) {
        ...
        if (!storedUrl) {
            throw new NotFoundException(`${kind === 'avatar' ? 'Profile photo' : 'Signature'} not found`);
        }
    ...
    private async uploadImage(
        ...
        if (!(PORTAL_PROFILE_ALLOWED_IMAGE_MIMES as readonly string[]).includes(mime)) {
    ...
    private mapToDto(user: NonNullable<Awaited<ReturnType<ProfileRepository['findPortalProfile']>>>): PortalProfileDto {
        ...
        const profileComplete = hasCoreFields && (!isDirector || Boolean(user.signature_url));
