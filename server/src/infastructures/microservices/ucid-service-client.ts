...
export class UcidServiceClient {
  private readonly baseUrl =
    process.env.UCID_SERVICE_URL ?? 'http://localhost:8003';
  ...
  async validateId(
    ...
    form.append(
      'front',
      toImageBlob(frontImage, frontMimetype, frontFilename),
      frontFilename,
    );
    form.append(
      'back',
      toImageBlob(backImage, backMimetype, backFilename),
      backFilename,
    );
    ...
    if (response.status === 503) {
      throw new Error(
        payload.message ??
          'UCID validation service is not ready. Train models first.',
      );
    }
    ...
    if (!response.ok || !payload.ok) {
      return {
        ...
        message:
          payload.message ?? 'Uploaded images are not valid UCLM ID cards',
      };
    }
